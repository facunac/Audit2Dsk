...
def get_campo(pName, pType, pBuffer):
	idx=pBuffer.find(pName)
	if idx >= 0:
		fdx=pBuffer[idx+pName.len+1:].find(asc(0))
		if fdx >= 0:
			if pType == 1:
				return pBuffer[idx+pName.len+1:fdx]
			else:
				return ord(pBuffer[idx+pName.len+1:fdx])
	return -1

cn = db.connect(host = "base.autentia.cl", user = "autentia", passwd = '_voyager.', port=3306,
                db = "autentia" )
cr = cn.cursor()

#cr.execute("SELECT NroAudit,Datos,Registrado FROM TAudit WHERE Datos not like 's3%' order by registrado desc limit 100")
cr.execute("SELECT NroAudit,Datos,Registrado FROM TAudit WHERE NroAudit = 'IMED-T296-TFDV-4PED'")
...
while rows:
	...
	row =  dict(zip(LABELS,row))
        unpack_taudit(row)
	pDat = row['Datos']
	print  get_campo("Dedo.inx",1,pDat)
	#idx=pDat.find("Dedo.inx")
        #print ord(pDat[idx+9:idx+10])
        #print ord(pDat[idx+10:idx+11])
    rows = cr.fetchmany(size = MAXROWS)
